...
| --- | --- | --- | --- |
| Orchestrator | `gpt-5.6-sol` | high | active session policy |
| Architect | `gpt-5.6-sol` | high | workspace-write for knowledge artifacts |
| Explorer | `gpt-5.6-terra` | medium | read-only |
| Implementer | `gpt-5.6-terra` | high | workspace-write |
| Evaluator | `gpt-5.6-terra` | high | workspace-write for evidence artifacts |
...
Use Sol/high for ambiguity, architecture, high-impact design, cross-domain
tradeoffs, security boundaries, data integrity, public contracts, complex failure
analysis, critical Eval design, review adjudication, and convergence. Use Terra
for strong everyday implementation and tool execution. Use Luna for clear,
...
self-justification is not authoritative.
